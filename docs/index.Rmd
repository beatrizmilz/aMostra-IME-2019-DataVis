...
```{r, out.width="30%", fig.cap="<center><b>Ilustração por Allison Horst - Twitter: <a href='https://twitter.com/allison_horst/'> @allison_horst</a> </b></center>"}
...
<!-- Base de Dados dfhair + boxplot -->
---
class: middle

# Base de Dados dfhair
...
```
```{r, echo=TRUE}
#Carregando o pacote que extrai dados do mercado de ações
...
l.out <- BatchGetSymbols(tickers = empresas,
...
ggplot(meudataset, aes(x = ref.date, 
                       y = volume)) +
  geom_line()
```
]
...
.left-code[
```{r plot-exemplo-heatmap, echo=TRUE, fig.show="hide", out.width="90%"}
library(viridis)
counts <- diamonds %>% 
  count(color, cut)

ggplot(counts, aes(color, y = cut, fill = n)) +
    geom_tile() +
    scale_fill_viridis() # use a better color palette is one option for better viz
...
ggplot(counts2, aes(color, cut, fill = prop)) +
    geom_tile() +
    scale_fill_viridis()
```
]
...
.right-plot[
![](`r knitr::fig_chunk("plot-exemplo-heatmap", "png")`)
]
...
<!-- Base de Dados XXX + Correlagrama -->
---
class: middle

# Base de Dados XXX

```{r desc-exemplo-corr, echo=TRUE, out.width="50%"}
"descritiva dos dados"
```
...
# Base de Dados diamonds + Correlagrama
...
ggpairs(small_diamonds, columns = c(1,3,5))
```
]
...
.left-code[
```{r plot-exemplo-plotly, echo=TRUE, fig.show="hide", out.width="90%"}
library(plotly)
...
.right-plot[
![](`r knitr::fig_chunk("plot-exemplo-plotly", "png")`)
]




<!-- Base de Dados XXX + esquisse -->
---
class: middle

# Base de Dados XXX

```{r desc-exemplo-esquisse, echo=TRUE, out.width="50%"}
"descritiva dos dados"
```
...
.left-code[
```{r plot-exemplo-esquisse, echo=TRUE, fig.show="hide", out.width="90%"}
library(esquisse)
```
]

.right-plot[
![](`r knitr::fig_chunk("plot-exemplo-esquisse", "png")`)
]
